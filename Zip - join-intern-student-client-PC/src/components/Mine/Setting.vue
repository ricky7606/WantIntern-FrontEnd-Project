<template lang="html">
  <div class="mine-setting">
    ...
    <div class="content">
      <div class="row setting-item" v-show="dataCopy.account">
        已绑定手机号码
        <span class="phone-number">
          {{dataCopy.account && dataCopy.account.phone}}
        </span>
      ...
      <div class="row setting-item" v-if="dataCopy.account">
        ...
        <span class="wechat-account">
          {{dataCopy.account.unionId ? '已绑定' : '未绑定'}}
        </span>

        <span class="btn pointer-on change" @click="bindingWechat">
          {{dataCopy.account.unionId ? '更改微信号' : '绑定微信号'}}
        </span>

        <span class="btn-cancel btn" v-if="isWechatWindowShow" @click="hideWechatBindingWindow">关闭二维码</span>
        <span class="btn-cancel btn" v-if="isWechatWindowShow && dataCopy && dataCopy.account && dataCopy.account.unionId" @click="setEmpty">取消绑定</span>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      setEmpty () {
        Req.Put(
          ReqUrl.OtherSubUrl.putWechatOpenIdToAccount(0, ''),
          null,
          res => {
            alert('操作成功')
