<template lang="html">
  <div class="news-list xsx-form-theme">
    ...
    <div class="list align-center">
      <table v-show="!isRichTextEditing">
        ...
        <template v-for="newsItem in list">
          <tr>
            <td class="id">{{newsItem.title}}</td>
            <td class="id">{{newsItem.addr}}</td>
            <td class="title">{{parseDate(newsItem.startDate)}} 至 {{parseDate(newsItem.endDate)}}</td>
            <td class="author">{{newsItem.startTime}} - {{newsItem.endTime}}</td>
            <td class="author">{{newsItem.salary}}{{newsItem.unit}}</td>
            <td class="author">{{newsItem.minDays}} 天</td>
            <td class="author">{{judgeValue(newsItem.retention)}}</td>
            <td class="author">{{mapReqEdu[newsItem.reqEdu]}}</td>
            <td class="author">{{newsItem.internCnt}}</td>
            <td class="author">{{newsItem.candidateCnt}}</td>
    ...
    <div class="company-info-edit">
      <com-resume-list :current="editingContent"
                         @close="closeSubpage"
                         @update="updateData"
...
<script>
  ...
  import ComResumeList from '@/components/Admin/ComResumeList.vue'
  import { ParsePagination } from '@/utils/helper-functions.js'
  import { mapReqEdu, eduEnumList, PositionEnum, mapReqEduNum } from '@/components/Position/PositionEnum.js'
  ...
  export default {
    ...
    methods: {
      ...
      getData ({pageIndex, pageSize}) {
        ...
        let url = ReqUrl.Company.getPositionsOfCompany(id)
        Req.Get(url, {
