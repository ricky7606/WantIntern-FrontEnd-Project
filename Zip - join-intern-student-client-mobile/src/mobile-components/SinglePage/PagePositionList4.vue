    ...
    <div class="content">
      ...
      <div class="list">
        <template v-for="item in list">
          <div class="list-item" @click="lookupPosition(getPosition(item).id)">
            <div class="row flex-row flex-between title">
              ...
              <span class="status">
                <span :class="{gray: shouldGray(tag)}" v-for="tag in translateStateText(item.state, item.commentStu)"
                  @click.stop.prevent="handleItem(tag, item)">
                  {{ tag }}
                </span>
              </span>
            </div>
            ...
            <div class="row reqs times">
              实习时间：{{ parseDateString(item.offerTime) }} 至 {{ parseDateString(item.endTime) }}
            </div>
          </div>
        </template>
      </div>
