<template lang="html">
  <div class="page-message-detail">
    <!-- 待学生确认 -->
    <div class="content" v-if="resumeStatus === 'WAIT_STU_CONFIRM'">
    ...
    <!-- 评价公司 -->
    <page-rate-company
      v-if="resumeStatus === 'ENDED'"
      @submit="ratingSubmit"
      :company-title="resumeContent.position.company.name" />
...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      this.loadResumeItem(id)
      // this.loadResumeItem(id, resumeId => {
    ...
    methods: {
      ratingSubmit (data) {
        console.log(data)
        let resumeId = this.resumeContent.id
        let result = data
        ...
        Req.Put(editUrl, {
          commentStu: '学生已评价',
          state: this.resumeContent.commentComp === '企业已评价' ? 'COMMENTED' : 'ENDED',
        }, res => {
          ...
          Req.Post(url, {
            ...
            Toast('评价成功')
            console.log(err.response.data.userMessage)
          })
        })
      },
    },
